...
| LEARN-030 | Resilience singletons must persist | Critical | Rate limiter/circuit breaker never trip | New instances per call reset state | `def tool(): limiter = RateLimiter()` | Module-level singleton with `_get_limiter()` accessor | All @tool functions with resilience patterns |
| LEARN-031 | TYPE_CHECKING for forward refs | Low | Ruff F821 undefined name | Forward refs without import cause lint errors | `_client: "Client"` | `if TYPE_CHECKING: from mod import Client` | All forward reference type hints |
...
| LEARN-028 | Pre-push hooks also run checks | Med | `git push` fails with same errors as commit | Pre-push hook runs same pre-commit checks including mypy | `git push` blocked by untracked file errors | Use `SKIP=mypy git push` when security checks pass but mypy fails on unrelated files | All pushes when codebase has type errors |
| LEARN-029 | SDK tool return format mandatory | High | Agent fails to process tool output | SDK tools must return `{"content": [...], "is_error": bool}` not plain dicts | `return {"email": result}` | `return {"content": [{"type": "text", "text": json.dumps(data)}]}` with `is_error: True` on failure | All `@tool` decorated functions |
| LEARN-030 | Resilience singletons must persist | Critical | Rate limiter/circuit breaker never trip | Creating new instances per tool call resets state, defeating purpose | `def tool(): limiter = RateLimiter(); limiter.acquire()` | Module-level singleton: `_limiter = None; def _get_limiter(): global _limiter; if not _limiter: _limiter = RateLimiter(); return _limiter` | All @tool functions with rate limiting or circuit breakers |
| LEARN-031 | TYPE_CHECKING for forward refs | Low | Ruff F821 undefined name errors | Forward references as strings without import cause linting errors | `_client: "SomeClient \| None" = None` | `from typing import TYPE_CHECKING; if TYPE_CHECKING: from module import SomeClient` | All modules with forward reference type hints |
| LEARN-032 | SerperClient uses `organic` not `organic_results` | High | `AttributeError` at runtime | SerperSearchResult dataclass uses `organic` attribute, not `organic_results` | `for item in result.organic_results:` | `for item in result.organic:` - Check integration client source for correct attribute names | All code using SerperClient search results |
| LEARN-033 | Optional dep type ignores flip-flop | Low | `Unused "type: ignore"` OR `import-not-found` | MyPy finds module when installed locally but not in CI; type ignores needed for optional deps become "unused" when dep installed | `import anthropic  # type: ignore[import-not-found]` shows unused when anthropic installed | Accept the warning OR remove ignore and let CI handle it; `SKIP=mypy` for commits when security passes | All lazy imports of optional dependencies (anthropic, etc.) |
| LEARN-034 | isinstance union syntax | Low | `UP038 Use X \| Y instead of (X, Y)` | Ruff UP038 prefers `isinstance(x, A \| B)` over `isinstance(x, (A, B))` | `isinstance(score, (int, float))` | `isinstance(score, int \| float)` | All isinstance checks with multiple types |
| LEARN-035 | SDK import-untyped obsolete | Low | `Unused "type: ignore" comment [unused-ignore]` | SDK added stubs; `# type: ignore[import-untyped]` no longer needed | `from claude_agent_sdk import tool  # type: ignore[import-untyped]` | `from claude_agent_sdk import tool` | All SDK imports |
| LEARN-036 | ProcessError constructor | Med | `TypeError: unexpected keyword argument 'stdout'` | ProcessError takes `(message, exit_code, stderr)` not stdout | `ProcessError(exit_code=1, stdout="", stderr="err")` | `ProcessError(message="msg", exit_code=1, stderr="err")` | SDK exception testing |
