...
approval_content_type_enum = ENUM(
    "budget", "document", "content", "custom", name="approval_content_type", create_type=False
)
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE approval_action AS ENUM ('approve', 'disapprove', 'edit', 'cancel', 'expire', 'resubmit', 'notify')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "content_type", approval_content_type_enum, nullable=False, server_default="custom"
        ),
    ...
    op.create_index(
        "ix_approval_requests_telegram_message_id", "approval_requests", ["telegram_message_id"]
    )
    op.create_index(
        "ix_approval_requests_approver_created", "approval_requests", ["approver_id", "created_at"]
    )
    op.create_index(
