...
class TestAutoboundCallEndpoint:
    """Tests for call_endpoint method (future-proofing)."""

    async def test_call_endpoint_post(
        self, client: AutoboundClient, mock_http_client: MagicMock
    ) -> None:
        """Should call endpoint with POST method."""
        mock_response = MagicMock()
        mock_response.status_code = 200
    ...
    async def test_call_endpoint_get(
        self, client: AutoboundClient, mock_http_client: MagicMock
    ) -> None:
    ...
    async def test_call_endpoint_default_post(
        self, client: AutoboundClient, mock_http_client: MagicMock
    ) -> None:
        """Should default to POST method."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        await client.call_endpoint("/endpoint")

        call_kwargs = mock_http_client.request.call_args[1]
        assert call_kwargs["method"] == "POST"

    async def test_call_endpoint_invalid_method_raises(self, client: AutoboundClient) -> None:
        """Should raise ValueError for unsupported HTTP method."""
        with pytest.raises(ValueError, match="Unsupported HTTP method"):
            await client.call_endpoint("/endpoint", method="INVALID")

    async def test_call_endpoint_case_insensitive_method(
        self, client: AutoboundClient, mock_http_client: MagicMock
    ) -> None:
        """Should handle method case-insensitively."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.text = '{"ok": true}'
        mock_response.json.return_value = {"ok": True}
        ...
        await client.call_endpoint("/endpoint", method="post")
